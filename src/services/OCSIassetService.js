...
export const OCSIfetchAssetsService = async (params = {}) => {
  try {
    if (navigator.onLine) {
      const queryObj = {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/ocsi/asset/get/all?${new URLSearchParams(queryObj)}`,
...
export const OCSIfetchAllAssetsService = async () => {
  try {
    if (navigator.onLine) {
      const res = await fetch(
